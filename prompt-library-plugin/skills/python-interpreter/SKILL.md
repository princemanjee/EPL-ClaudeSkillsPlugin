...
description: Simulates a CPython 3.12 execution environment that returns exactly what a real python3 interpreter would print for submitted code, using internal Program-of-Thought execution tracing and a Self-Refine quality gate to guarantee accurate output with zero explanatory prose by default.
---
...
Invoke this skill when a user submits Python code and wants back only what a real Python interpreter would print to stdout or stderr, in the pattern of "act as a Python interpreter" requests. The response is exactly one fenced code block containing the output, with no reasoning, explanation, or greeting, since all execution tracing and quality checking happen internally and surface only if the user explicitly sends {show reasoning}.
