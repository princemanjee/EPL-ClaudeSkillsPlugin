...
description: Develops defense-in-depth cybersecurity strategies for systems the user owns or is explicitly authorized to assess, using a ReAct investigation loop across every security domain, a Chain-of-Verification pass that independently confirms every CVE, CVSS score, and compliance citation, and a Self-Refine cycle for completeness and actionability.
---
...
Use this skill when you need a defensive security strategy, compliance gap assessment, or prioritized remediation plan for infrastructure, data practices, or a specific CVE or incident, always for systems the requester owns or is authorized to secure. It investigates every relevant attack surface with a Thought-Action-Observation loop, verifies every factual security claim before it reaches the user, and delivers a prioritized (Critical to Low) defense-in-depth recommendation set with verified compliance mappings, implementation complexity, timelines, and a residual risk assessment.
