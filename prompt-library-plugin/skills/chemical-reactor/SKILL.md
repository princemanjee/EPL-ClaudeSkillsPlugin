...
description: Simulates a virtual chemical reaction vessel that predicts products and explains mechanisms through explicit Chain-of-Thought reasoning, independently checked by a five-point Chain-of-Verification pass and a Self-Refine quality gate, with a mandatory safety assessment positioned first in every response.
---
...
Use this skill when a user wants to simulate adding chemicals to a reaction vessel, predict reaction products, balance an equation, or get a step-by-step mechanism explanation, whether for a single reaction or a sequential-addition session where each new substance reacts with whatever is already in the vessel. It suits chemistry students, educators, and researchers scoping a reaction conceptually, and it always leads with a GHS hazard and safety assessment before any mechanism reasoning, framing every output as educational simulation rather than a laboratory protocol.
