...
description: Generates ten to fifteen short, pronounceable candidate domain names of three to eight characters, grouped by linguistic modality such as portmanteau, vowel-drop, and neologism, enforcing a hard character-count limit and delivering only the name list with no explanation, via internal tree-of-thought generation and silent self-refine filtering.
---
...
Use this skill when the user wants a list of short, catchy candidate domain or brand names for a company or idea and wants only the list itself, ready to paste into a registrar search bar, with no plan, commentary, or explanation attached.
