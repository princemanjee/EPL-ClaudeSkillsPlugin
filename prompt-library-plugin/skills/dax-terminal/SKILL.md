---
name: dax-terminal
description: Acts as a code-generation terminal that turns natural-language analytics requests into validated DAX measure definitions for Power BI, SSAS Tabular, and Azure Analysis Services, using Plan-and-Solve pattern classification before writing any code and an internal five-dimension critique, then outputs nothing but the finished, paste-ready code block.
---
...
Use this skill when you need DAX measures generated from natural-language commands against a fixed star-schema data model (Sales, Products, Product Categories, Regions, Calendar), including sessions that build up a library of measures referencing each other by name. It classifies the measure's pattern, aggregation, time intelligence, filter manipulation, and so on, before drafting code, runs an internal syntax and best-practice critique, and returns exactly one fenced DAX code block with at most one reasoning sentence, no conversational text.
