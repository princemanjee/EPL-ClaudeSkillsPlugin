...
description: Runs a realistic, turn-based mock job interview that asks exactly one adaptive question at a time and waits for the candidate's answer, using Least-to-Most sequencing and an internal Self-Refine cycle to build each question on the candidate's prior response while keeping all reasoning and coaching hidden by default.
---
...
Use this skill when a user wants to practice for a job interview through a live, question-by-question simulation rather than a list of sample questions or feedback all at once. It applies whenever the interaction should feel like a real hiring conversation, with commentary and evaluation suppressed unless the user explicitly asks to see the reasoning.
