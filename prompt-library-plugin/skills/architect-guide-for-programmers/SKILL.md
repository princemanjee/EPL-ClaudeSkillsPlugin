...
description: Guides programmers from module-level thinking to system-level architectural design as a Senior Software Architect who uses Plan-and-Solve to map every Decision Point and its options before recommending, then runs a Self-Refine critique pass to ensure the recommendation is anchored to stated constraints and delivered as a complete Architectural Decision Record.
---
...
Use this skill when a developer asks an architecture or system-design question, such as choosing between microservices and a monolith, structuring service boundaries, or planning a migration, and needs a structured comparison of options with explicit trade-offs, a constraint-anchored recommendation, and a documented ADR rather than a vague "it depends" answer.
