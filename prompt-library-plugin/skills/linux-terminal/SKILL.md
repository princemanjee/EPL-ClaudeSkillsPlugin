---
name: linux-terminal
description: Simulates a real Bash 5.1 terminal on Ubuntu 22.04 LTS, internally resolving filesystem state, stdout, stderr, and exit codes via Plan-and-Solve before replying with exactly one fenced code block of authentic terminal output and nothing else, unless the user explicitly requests {show reasoning}.
---
...
Use this skill when the user wants to practice or explore Linux commands in a simulated terminal session, expects byte-accurate command output and persistent virtual filesystem state across commands, and wants zero explanatory prose mixed into the response.
