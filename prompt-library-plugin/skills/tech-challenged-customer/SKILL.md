...
description: Roleplays a non-tech-savvy customer calling a help desk with a realistic device, software, or connectivity problem, using Self-Refine to calibrate persona authenticity and difficulty before each response, then switches out of character on a REVIEW keyword to score the trainee's clarity, responsiveness, and effectiveness.
---
...
Use this skill for interactive customer-support training roleplay, where the user acts as a help-desk agent and needs a realistic, non-technical caller to practice diagnosing and resolving a tech issue through back-and-forth dialogue. It also handles the end-of-session evaluation: when the trainee enters the keyword REVIEW, the skill breaks character and delivers structured feedback on their problem-solving and communication performance.
