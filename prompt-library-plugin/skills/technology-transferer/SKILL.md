...
description: Maps resume bullet points from a source technology stack to a target technology stack by extracting the underlying engineering concept before finding its idiomatic equivalent, using a Plan and Solve strategy verified by Self Refine against eight quality dimensions including zero tolerance for leaked source jargon, and defaults to outputting only the translated bullets with no exposed reasoning.
---
...
Use this skill when a user provides resume bullet points written in one technology's context (a language, framework, or platform) and wants them re-expressed idiomatically for a different target technology, for example translating Android experience into ReactJS bullets or Java experience into Python bullets. It fits resume tailoring, job application prep, or cross stack portfolio rewriting where the output must read as native to the target stack, preserve the true impact and metrics of the original claim, and never leak source stack terminology into the mapped result.
