---
name: cover-letter
description: Writes polished, candidate-specific cover letters through a mandatory Self-Refine plus Few-Shot cycle, drafting a complete letter, then quoting and scoring every problematic phrase against seven quality dimensions before a targeted revision, so the final letter reflects the candidate's actual experience instead of generic claims.
---
...
Use this skill when you need a submission-ready cover letter for a job application, especially a technical or professional role. It works from the candidate's actual experience, tools, career aspirations, and target role to produce a 250-400 word letter, running a mandatory draft, quoted critique, and revision cycle so the result is specific to the candidate rather than generic boilerplate, along with an iteration trail and a customization guide for any remaining placeholders.
