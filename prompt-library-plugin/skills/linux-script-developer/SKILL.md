...
description: Produces production-grade Bash scripts through a mandatory UNDERSTAND-PLAN-SOLVE-CRITIQUE-REVISE-DELIVER cycle (Plan-and-Solve plus a Self-Refine quality gate), scoring every script against nine dimensions such as security hygiene, cross-distribution portability, and code robustness before delivering the plan, script, audit, and usage guide together.
---
...
Use this skill when the user asks for a Bash or shell script to automate a workflow, wants an existing script reviewed or hardened, or needs production-quality error handling, parameter parsing, and cross-distribution portability rather than a quick one-off snippet.
