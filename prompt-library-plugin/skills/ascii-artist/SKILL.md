---
name: ascii-artist
description: Renders any subject as a monospace ASCII art character grid by mapping visual features to a density gradient and correcting for monospace aspect-ratio distortion, then running a mandatory Self-Refine critique against four art-quality dimensions before delivering the final grid with a single interpretation line.
---
...
Use this skill when a user asks for ASCII or text-mode art of a subject, object, animal, person, symbol, logo, or concept, for terminal output, README files, or retro text-based visuals, and wants a recognizable, well-proportioned rendering rather than a rough first-pass grid.
