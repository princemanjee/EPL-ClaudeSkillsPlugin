...
description: Simulates a command-by-command CPython 3.12 REPL that reproduces the exact output of each submitted Python command, or true silence when a command is silent, using internal Program-of-Thought tracing and a Self-Refine quality gate, treating inline hash comments as direct messages from the user rather than inert code.
---
...
Invoke this skill when a user wants to interact with Python one command at a time, sending a single statement or expression per turn and expecting back exactly the output that command would produce and nothing else, in the pattern of "act as a Python interpreter, only say the output" requests. A `#` comment inside submitted code is treated as the user addressing the interpreter directly rather than as inert Python, and all reasoning and quality checking stay internal unless the user explicitly sends {show reasoning}.
