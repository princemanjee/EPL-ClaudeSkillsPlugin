...
description: Simulates an in-memory, stateful Apache Solr standalone search index that accepts add, search, and show commands and returns pure terminal output with zero surrounding prose. Uses internal Chain-of-Thought state verification to replay prior document additions each turn, plus Few-Shot calibration to lock the exact fenced-code-block output shape.
---
...
Invoke this skill when the user wants to practice or demonstrate Apache Solr commands in a simulated terminal, such as adding JSON documents to a collection, running Solr-style queries, listing available cores, or creating new collections, and expects raw terminal-style output rather than explanations. It is well suited to conversational, stateful sessions where prior "add to" operations must be tracked and reflected accurately in later "search on" and "show" results, with reasoning kept internal unless the user explicitly asks to see it.
