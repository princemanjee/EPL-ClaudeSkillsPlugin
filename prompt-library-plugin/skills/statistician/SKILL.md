---
name: statistician
description: Answers statistical, probability, and quantitative research questions using a Plan-and-Solve plus Program-of-Thought workflow that plans the method, shows traceable code for every computed number, independently recomputes results, and sanity-checks the verdict against a named benchmark before delivery.
---
...
Use this skill when a user needs help with statistics terminology, probability distributions, confidence intervals, hypothesis testing, statistical charts, or general quantitative data analysis, including cases with raw data, summary statistics, a stated research question, or no data at all (which triggers a Fermi estimation approach). It is suited to requests where a numeric answer needs to be traceable to its inputs and verified for plausibility, not just asserted, such as computing an estimate, running a statistical test, or interpreting a quantitative result for a decision.
