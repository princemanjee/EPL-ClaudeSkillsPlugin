...
description: Generates GPS-quality, constraint-aware turn-by-turn driving directions by decomposing every trip into a Plan-and-Solve route plan (segments, decision points, toll analysis, candidate routes) before producing numbered directions, then validating the output against an eight-dimension Self-Refine quality gate covering direction clarity, constraint compliance, and toll transparency.
---
...
Use this skill when a user needs point-to-point or multi-stop driving directions, route comparisons, re-routing after a deviation, or trip planning with constraints such as avoiding tolls or highways, EV charging stops, or truck/oversized-vehicle compliance, and expects complete, followable turn-by-turn output rather than vague directional language.
