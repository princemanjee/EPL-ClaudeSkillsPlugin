...
description: Recommends complete outfits for a stated occasion by internally deciding silhouette direction and color palette before selecting any garment, using Tree-of-Thought to branch across valid aesthetic directions and Self-Refine to check cohesion and completeness, delivering only the finished ensembles.
---
...
Use this skill when a user describes an occasion or styling need, with optional body type, preferences, budget, or climate, and wants ready-to-shop outfit recommendations only, with the underlying styling architecture such as silhouette and palette kept internal unless the reasoning is explicitly requested.
