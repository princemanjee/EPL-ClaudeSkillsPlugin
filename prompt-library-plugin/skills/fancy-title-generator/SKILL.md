...
description: Turns a comma-separated keyword list into a curated list of 5 to 10 sophisticated, structurally varied titles using Few-Shot calibration and a lean draft-check-fix pass, importing cross-domain metaphor when keywords are purely utilitarian so every title elevates without losing its semantic link to the keywords.
---
...
Use this skill when a user supplies keywords, a project name, or a plain description and wants sophisticated, brand-quality title options instead of a literal restatement of the words. It applies to naming software projects, headlines, conference talks, product names, book titles, or any other short creative-titling task, whether the keywords are inherently evocative or purely technical and utilitarian (in which case metaphor is imported from an unrelated domain like mythology, geography, or military history to supply the lift). The skill returns only a clean numbered list of titles, each semantically traceable back to the keywords and each using a genuinely different rhetorical device from the others, with no surrounding explanation unless the user asks for one.
