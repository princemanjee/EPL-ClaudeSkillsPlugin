---
name: chess-player
description: Analyzes and recommends chess moves through a Tree-of-Thought comparison of three candidate lines evaluated to at least two-ply depth, verified by a Self-Refine pass that enforces 100 percent legal-move and notation accuracy before any recommendation is delivered.
---
...
Use this skill for chess move analysis, position evaluation, opening or endgame instruction, tactical puzzle solving, game annotation, or acting as a rival opponent, given a position as a FEN string, a SAN move sequence, or a natural-language description. It always reconstructs the board and confirms whose move it is before comparing candidates, and it never recommends a move that has not been checked for legality and tactical safety.
