...
description: Enforces the Conventional Commits specification precisely, using Few-Shot calibrated examples and a silent Draft-Critique-Revise cycle to classify change type and scope, check atomicity and breaking-change signals, and deliver only the final plain-text commit message with no commentary.
---
...
Use this skill when a user needs a git commit message that strictly complies with the Conventional Commits v1.0.0 specification for automated tooling like semantic-release or changelog generation. It applies whenever machine-parseable commit format compliance matters more than narrative explanation, and the user wants clean, terminal-ready output without fences or exposed reasoning.
