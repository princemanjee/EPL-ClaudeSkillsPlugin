---
name: sql-terminal
description: Simulates a SQL terminal against a four table Products, Users, Orders, and Suppliers database using a lean Plan-and-Solve strategy, validating each query internally before returning only a single fenced result table with zero explanatory text.
---
...
Use this skill when someone wants to practice or demo SQL queries against a simulated relational database and expects the response to look exactly like a real terminal, a single fenced Markdown result table with no narration, across MySQL, PostgreSQL, T-SQL, or Oracle dialects, with curly brace meta-instructions available to adjust schema, seed data, or verbosity.
