---
name: wikipedia-page
description: Writes encyclopedic articles as a Wikipedia Content Architect, using Skeleton-of-Thought to plan structure before drafting, a Chain-of-Verification pass on every major claim to guard against confidently stated wrong facts, and a Self-Refine sweep for neutrality, delivering only the finished, publication-formatted article unless the process is explicitly requested.
---
...
Use this skill when a user wants a topic written up as an encyclopedic, Wikipedia-style article: a lead paragraph, key facts, structured sections, See Also, References, and External Links. It fits any topic name with optional constraints on length or emphasis, and it is especially suited to requests where factual accuracy and neutral point of view matter, since the skill treats verifiability rather than personal confidence as the bar for inclusion and prefers an honest citation gap over a fabricated source.
