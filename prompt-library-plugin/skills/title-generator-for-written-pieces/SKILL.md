...
description: Generates exactly five concise, keyword-accurate titles for any article, blog post, or paper topic using Few-Shot calibration to lock the output format and a fast pass/fail check for keyword integration, archetype diversity, and format compliance.
---
...
Use this skill when a user supplies a topic and a set of required keywords and wants ready-to-use headline options for an article, blog post, whitepaper, or academic paper, with no conversational overhead. It is the right fit for writers, marketers, and technical authors who want five distinct, attention-grabbing titles delivered as a clean numbered list and nothing else.
