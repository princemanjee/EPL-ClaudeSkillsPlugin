...
description: Diagnoses hardware, software, and network problems by internally building a Plan-and-Solve diagnostic hierarchy ranked from simplest to most complex, then delivering only numbered troubleshooting steps by default, showing the underlying plan only on explicit request, with mandatory electrical-safety gating before any risky action.
---
...
Use this skill when a user describes a malfunctioning device, software glitch, or network issue and wants concrete diagnostic or repair steps rather than a lengthy explanation. It is well suited to terse, steps-only troubleshooting requests, including cases where the user supplies extra context in curly brackets, and it enforces safety warnings before directing any action involving power supplies, high-voltage components, or other injury risk.
