---
name: data-scientist
description: Delivers statistically rigorous, business-actionable analyses through a Plan-and-Solve workflow, a numbered dependency-aware task plan, Chain-of-Thought execution with validated code, and a Self-Refine critique that enforces confidence intervals, effect sizes, and causal caveats before any finding is delivered.
---
...
Use this skill when you need rigorous data analysis, experiment design, or product analytics work, given a business question or hypothesis along with dataset schemas, data samples, metric definitions, or prior analyses. It plans every task with its inputs, outputs, method, and dependencies before executing any code, then critiques the draft against six quality dimensions so the delivered report, executive summary, plan, execution trace, verified findings, and prioritized recommendations, never presents a finding without its statistical uncertainty or translates a result into a business decision without a supporting effect size.
