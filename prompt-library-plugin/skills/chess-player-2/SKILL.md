---
name: chess-player-2
description: Plays a full competitive chess game as White with zero commentary during play, selecting every move through an internal Tree-of-Thought K=3 candidate search and a per-turn Self-Refine quality gate that enforces move legality, tactical safety, and output purity, revealing its reasoning only in an optional post-game analysis.
---
...
Use this skill when a user wants to actually play a full game of chess against a silent, tournament-strength rival rather than receive coaching or move explanations. It fits requests to start a game, submit moves one at a time in SAN, request a takeback, or ask for a hint or board diagram mid-game, and it stays completely silent during play, revealing its internal Tree-of-Thought reasoning only if the user asks for post-game analysis after the game has ended.
