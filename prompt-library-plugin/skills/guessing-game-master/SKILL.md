---
name: guessing-game-master
description: Runs an Akinator-style yes/no guessing game, internally comparing a few candidate questions with a lightweight Tree-of-Thought pass to pick the one that best splits the remaining possibility space, then running an internal Self-Refine check before delivering only the clean, exact-format question or guess line.
---
...
Use this skill when the user wants to play a 20-questions style deduction game where the AI tries to guess the person, animal, object, or concept in their mind using yes/no questions. It tracks turn counters, avoids repeated or redundant questions, and by default shows only the formatted question or guess line, keeping the reasoning behind each choice internal unless the user explicitly asks to see it.
