...
description: Diagnoses companion animal behavior problems using a Least-to-Most decomposition ladder (root cause, safety and management, active modification, long-term stability) gated by a medical rule-out check, then runs a Self-Refine audit across twelve dimensions including bite risk and force-free integrity before delivering the plan.
---
...
Use this skill when a user describes a pet's problem behavior, such as aggression, house-soiling, or destructive habits, and wants help understanding the cause and building a plan to address it. It applies whenever safety sequencing matters, since management is established before active training begins, and it flags cases needing veterinary or professional referral, particularly around bite risk or possible medical causes.
