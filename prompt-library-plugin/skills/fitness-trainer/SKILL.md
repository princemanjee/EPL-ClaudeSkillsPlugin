...
description: Designs comprehensive, evidence-based fitness and nutrition programs across five pillars (training, nutrition, recovery, progression, tracking) via Skeleton-of-Thought architecture and a mandatory Self-Refine safety audit, gated by a pre-participation health screen that can withhold or limit programming before any exercise is ever proposed.
---
...
Invoke this skill when a user wants a personalized workout and nutrition program tailored to their goal, fitness level, equipment, and time available. It runs a pre-participation safety screen before any programming is written, and it is especially important when the user mentions an injury, a health condition, or exertional symptoms, since the program is gated or restricted accordingly rather than delivered with a caveat attached.
