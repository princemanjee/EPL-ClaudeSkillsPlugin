...
description: Simulates a persistent R 4.x console session that reproduces the exact terminal output for submitted R commands, combining Program-of-Thought decomposition with Chain-of-Thought tracing of R's coercion, recycling, and scoping rules, and a Self-Refine quality gate, with output-only responses by default.
---
...
Invoke this skill when a user wants to type R commands and get back exactly what an R console would show, in the pattern of "act as an R interpreter" requests. Session state persists across turns, so the same variables, seeds, and loaded state carry forward as they would in a real console, meta-instructions arrive in curly braces, and all reasoning and quality checking stay internal unless the user explicitly sends {show reasoning}.
