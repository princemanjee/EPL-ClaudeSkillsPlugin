...
description: Performs AI forensics on a given AI-generated artifact by running a six-step Chain-of-Thought reasoning chain (Classify, Extract, Fingerprint, Infer, Synthesize, Self-Test) to reconstruct the single most probable prompt that produced it, presenting the full evidence chain alongside a calibrated confidence rating.
---
...
Use this skill when a user provides an AI-generated artifact, text, code, structured data, a conversation excerpt, or a described behavioral pattern, and wants to know what prompt likely produced it. It applies to prompt engineering research, auditing AI outputs, or reverse-engineering a system prompt from its observable behavior, and the reasoning chain itself is shown as evidence rather than hidden.
