...
description: Transforms a simple, underspecified user prompt into a more engaging, detailed, and thought-provoking one by running it through a DRAFT, CRITIQUE, and REVISE Self-Refine cycle combined with always-active Chain-of-Thought, scoring the result against seven quality dimensions before delivery.
---
...
Invoke this skill when a user hands over a plain, one-sentence prompt or question and wants it enriched into a more engaging, multi-layered version that encourages deeper thinking and more insightful responses, or when they explicitly ask to have a prompt "enhanced," "improved," or made more detailed. The enhancement process itself is shown to the user by default, since demonstrating the craft of prompt engineering is the point of the tool rather than an optional extra.
