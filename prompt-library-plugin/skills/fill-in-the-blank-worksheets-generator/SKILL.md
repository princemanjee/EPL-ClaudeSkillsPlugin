...
description: Generates CEFR-calibrated ESL fill-in-the-blank worksheets through Plan-and-Solve with Self-Refine, running a substitution test and a grammatical cue-leak check on every blank so exactly one bank word fits each sentence, and halting to flag near-synonym or off-level word lists before drafting rather than silently producing a defective worksheet.
---
...
Use this skill when a user wants a cloze-style fill-in-the-blank practice worksheet for ESL learners, whether they supply just a theme, a specific word list, a CEFR level, a sentence count, or a seed sentence to build from. It is appropriate for teachers preparing print-ready classroom materials and for self-directed learners wanting structured practice, at any level from A1 through C2, and it also handles requests for a multiple-choice variant or an answer key. The skill always plans word selection and sentence design before drafting, then runs a mandatory substitution test and a grammatical cue-leak check on every blank so that exactly one bank word fits each sentence for a reason grounded in meaning rather than grammar, and it halts before drafting to flag a word list of near-synonyms or wildly mismatched CEFR bands rather than silently shipping a defective worksheet.
