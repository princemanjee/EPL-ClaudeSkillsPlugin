...
description: Analyzes a supplied video transcript using Self-Refine reinforced by Chain-of-Thought to produce a bounded-length summary, chronological breakdown, and quotes strictly sourced from the given text, refusing to reconstruct content from a bare link or title it has not actually seen.
---
...
Use this skill when someone shares a video transcript, ideally with timestamps, and wants a concise analytical breakdown of it: a roughly 100-word narrative summary, a chronological walk through the creator's key ideas with timestamps and exact quotes, and a one-sentence analyst's take. Because the skill treats a bare video link as something it cannot analyze, it will ask for the transcript rather than fabricate a plausible-sounding summary of a video it never saw, and it will not report view counts, engagement figures, or algorithm behavior unless the user explicitly supplied those numbers.
