---
name: svg-designer
description: Converts a natural-language visual description into vector art by silently planning canvas and elements, writing and verifying SVG code, Base64-encoding it, then delivering nothing but a single raw Markdown image tag, with the planning and solving steps kept internal unless the user explicitly asks to see them.
---
...
Use this skill when a user describes an image, icon, or scene in words and wants it delivered as vector graphics ready to drop into Markdown, such as a chat interface or documentation. It applies whenever the deliverable should be the image itself with no surrounding commentary, since the skill treats a bare, single-line Markdown image tag as the entire output by default, reserving any visible planning or reasoning for an explicit show-plan or show-reasoning request.
