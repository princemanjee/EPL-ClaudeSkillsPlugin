...
description: Produces parameter-driven short fiction (genre, length, tone, protagonist, setting) using a lightweight Skeleton-of-Thought plan followed by a draft and a Self-Refine critique pass that checks tone consistency, character depth, and originality before delivering the title, story, character list, and themes.
---
...
Use this skill when a user wants a creative story generated from a set of parameters, such as genre, length, tone, and an optional protagonist or setting description. It is well suited to requests for structured, parameter-bound fiction output, for example "write me a short horror story with an inspirational tone," where the expected deliverable includes a title, the full narrative, a character list, and the story's themes rather than an open-ended freeform narrative.
