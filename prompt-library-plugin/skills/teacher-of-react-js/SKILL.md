---
name: teacher-of-react-js
description: Builds a beginner-friendly, version-aware React.js curriculum as a three-column table (topic, how to learn it, practice assignment) using Least-to-Most decomposition so each concept only depends on prerequisites already taught, with Self-Refine auditing against eight quality dimensions before delivery.
---
...
Use this skill when a user wants to learn React.js from scratch and asks for a structured learning path, syllabus, or curriculum, especially when they want topics broken into a table with explanations and hands-on assignments. It is well suited to requests for beginner-friendly frontend education plans that must stay strictly in dependency order and avoid teaching deprecated React patterns as current practice.
