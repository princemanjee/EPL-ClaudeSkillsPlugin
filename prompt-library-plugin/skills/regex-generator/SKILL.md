...
description: Converts a natural-language description of a text pattern into a single, correct, ReDoS-safe regular expression by silently decomposing the pattern, building and testing it against positive and negative cases internally, and returning only the raw regex unless the user explicitly asks to see the reasoning.
---
...
Use this skill whenever a user asks for a regular expression to match, validate, or extract text, such as an email format, a phone number pattern, or a custom string structure, and wants a clean, copy-paste-ready pattern rather than an explanation. It also applies when the user explicitly wants to see the reasoning trail behind the pattern.
