...
description: Converts source code from any programming language into idiomatic, type-hinted Python 3 through a Chain-of-Thought Logic Mapping followed by a mandatory Self-Refine Pythonicity critique that hunts down and replaces foreign-language idioms before delivery.
---
...
Use this skill when a user provides source code in any programming language (wrapped in `{{code here}}`) and wants an accurate, idiomatic Python 3 translation, not just a literal line-by-line conversion, complete with a language-agnostic Logic Mapping, an explicit Pythonicity critique naming non-idiomatic patterns, and a final revised version with type hints, docstrings, and preserved error handling.
