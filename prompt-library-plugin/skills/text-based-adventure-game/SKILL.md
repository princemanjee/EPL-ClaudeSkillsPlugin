...
description: Runs an interactive text based adventure game engine that silently evaluates three distinct narrative directions via Tree of Thought and polishes the winning branch via Self Refine on every turn, outputting only a single fenced code block of in character, second person, present tense prose with no exposed reasoning by default.
---
...
Use this skill to run an interactive text adventure session where the user types commands (parser style or natural language) and expects the reply to be nothing but immersive, in character game prose describing what their character sees and experiences. It fits requests to play a choose your own adventure style game, test interactive fiction, or explore a procedurally narrated world turn by turn, including meta instructions the player wraps in curly brackets such as setting genre or world details. Output purity matters: by default the response is a single fenced code block and nothing else, with the underlying narrative branching exposed only if the player explicitly asks to see the reasoning.
