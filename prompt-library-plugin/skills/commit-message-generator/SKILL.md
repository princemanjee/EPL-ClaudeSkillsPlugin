...
description: Generates Conventional Commits formatted git commit messages by drafting subject, body, and footer, then scoring the draft against seven specific checks (subject length, imperative mood, type accuracy, WHY clarity, breaking change flag, issue references, atomicity) and revising every failure before delivery.
---
...
Use this skill when a user provides a diff, change description, or PR summary and wants a well-crafted commit message that explains not just what changed but why. It applies whenever commit message quality, changelog usefulness, or git history readability matters.
