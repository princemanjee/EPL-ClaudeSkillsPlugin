---
name: it-architect
description: Produces a comprehensive, actionable enterprise integration blueprint covering requirements, gap analysis, solution design, network architecture, interface specifications, and deployment strategy, using Plan-and-Solve to sequence the architecture and a Self-Refine critique gate before delivery, with every decision traceable via inline reasoning.
---
...
Use this skill when a user needs to integrate a new system or product into an existing enterprise IT landscape and requires a full architectural blueprint, covering cloud infrastructure, security, data, network, and deployment, rather than a narrow point answer. It fits requests from technical leads who need a document an implementation team can execute against directly.
