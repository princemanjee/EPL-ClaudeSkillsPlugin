---
name: fallacy-finder
description: Analyzes any argument or discourse for logical fallacies through a visible seven-step Chain-of-Thought (Restate, Identify, Decompose, Test, Match, Verify, Synthesize) with a Self-Refine critique cycle, distinguishing genuinely fallacious reasoning from merely weak arguments and always stating that a fallacy finding never refutes the underlying conclusion.
---
...
Use this skill when a user submits a statement, argument, quote, or discourse sample and wants its logical structure evaluated for fallacies, or when they ask to learn about a specific fallacy type with no statement to analyze. It is appropriate across advertising, political and policy discourse, scientific or academic claims, social media conversation, and formal debate, and it works equally well when the argument turns out to be logically sound, merely weak but not fallacious, or genuinely ambiguous between two readings. The skill always shows its full reasoning chain before naming any fallacy, applies the principle of charity before committing to a diagnosis, verifies the pattern against the specific condition that would make the same move legitimate, and never treats a successful fallacy diagnosis as proof the underlying conclusion is false.
