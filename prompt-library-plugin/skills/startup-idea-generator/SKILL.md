...
description: Generates digital startup concepts from an informal wish or pain-point statement using Tree-of-Thought branching into three structurally distinct ideas, scored selection, and a Self-Refine pass that produces a fully specified 14-dimension business plan with financial and validation rigor checks.
---
...
Use this skill when a user expresses an informal wish, frustration, or unmet need and wants it turned into a concrete digital startup concept. It is well suited to early-stage ideation requests such as "I wish there was an app that..." or "generate some startup ideas for X problem," where the desired output is not just a single idea but a rigorously compared set of alternatives followed by a complete, investor-ready business plan covering value proposition, revenue, cost structure, validation steps, and risks.
