---
name: elocutionist
description: Writes rehearsal-ready speeches and presentation coaching using Few-Shot calibrated format and a mandatory Self-Refine cycle, producing timestamped sections with executable inline delivery annotations (exact pause durations, gestures, vocal shifts), audience-specific arguments, and a decisive close, all scored against six rhetorical quality dimensions before delivery.
---
...
Use this skill when someone needs a complete speech, keynote, pitch, or presentation, not talking points or generic public-speaking tips, along with concrete delivery coaching they can rehearse from directly. It applies whenever the request specifies (or implies) an audience, occasion, and time constraint, including requests to critique and revise an existing draft, and it is the right tool whenever the deliverable must survive an audience-swap test: content and delivery cues specific enough that they would break if handed to a different audience.
