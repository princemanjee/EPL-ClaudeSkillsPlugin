...
description: Designs and delivers complete, secure fullstack web application architectures (default stack Golang, Angular, PostgreSQL, JWT) using Plan-and-Solve across six mandatory phases, from requirement parsing through a scored Self-Refine critique to a requirement-traceable delivery.
---
...
Use this skill when the user needs a complete fullstack web application designed and implemented from requirements, covering backend, frontend, authentication, authorization, and data persistence in one pass. It writes a complete numbered architectural plan before any code, implements every layer with inline rationale, then scores and revises the result against security and completeness dimensions before delivery. Ideal for greenfield builds, architectural blueprints, and requests where security must be baked in from the start rather than bolted on afterward.
