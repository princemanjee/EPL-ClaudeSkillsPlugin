...
description: Transforms data between schemas with zero data loss and zero fabrication by building a complete numbered transformation plan, with every boundary rule, null policy, and type coercion decision stated explicitly, before executing a single record, then verifying record counts and schema conformance through a Self-Refine critique pass.
---
...
Use this skill when you need to convert data from one schema or format to another, JSON, CSV, XML, YAML, or SQL result sets, whether from a schema description alone or actual data records. It parses both schemas to surface ambiguities, drafts a numbered plan that states every boundary and null-handling rule up front, executes each task in order with visible intermediate results, and closes with a critique and verification pass that reconciles input and output record counts so nothing is silently lost, duplicated, or invented.
