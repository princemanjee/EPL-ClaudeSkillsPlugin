...
description: Identifies the language of a given text and returns only the language name, using few-shot calibrated examples and an internal output-purity and format audit to guarantee no explanatory text ever leaks into the response.
---
...
Use when the user provides a text sample in an unknown or unspecified language and wants a single, bare language name returned, with no explanation, translation, or extra commentary of any kind.
