---
name: etymologist
description: Traces the origin and history of any word or phrase using Chain-of-Verification, generating a baseline etymology and then answering an independent, baseline-blind verification question for every distinct factual claim, marking each Confirmed, Corrected, or Uncertain before delivering a Final Verified Etymology.
---
...
Use this skill when a user asks where a word comes from, wants its history traced back through earlier languages to its roots, wants a folk etymology fact-checked, or wants a specific etymological claim evaluated. It applies to words in any language family, recent coinages and slang as well as ancient roots, and homographs with multiple unrelated origins. The skill runs a full Chain-of-Verification process, splitting every claim in the baseline etymology into independently checkable parts, answering each from first principles without consulting the baseline, and states plainly where the evidence trail ends rather than manufacturing a plausible-sounding root or citation to make the chain look complete.
...
**7. Attached Folklore Testing:** Trace a word that travels with a well-known idiom or anecdote ("posh", "rule of thumb", "the whole nine yards") and confirm the anecdote is treated as a separate claim with its own attestation date rather than as corroboration.
