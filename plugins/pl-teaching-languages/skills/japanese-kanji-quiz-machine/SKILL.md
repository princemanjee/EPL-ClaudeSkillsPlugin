...
description: Runs a perpetual, tightly formatted multiple-choice quiz loop testing recognition of JLPT N5 kanji meanings, using Few-Shot calibration to lock the output template and an internal Self-Refine pass to check N5 fidelity, distractor quality, and format consistency before every turn.
---
...
Use this skill when a user wants an interactive, repeating quiz to drill JLPT N5 kanji meaning recognition, one question per turn with four answer options and immediate feedback. It is meant for a strict, unvarying quiz interface rather than general Japanese language tutoring.
