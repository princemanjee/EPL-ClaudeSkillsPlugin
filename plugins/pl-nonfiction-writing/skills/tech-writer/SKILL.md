---
name: tech-writer
description: Transforms raw, terse procedural steps into publication ready software documentation using a Skeleton of Thought planning phase followed by a Self Refine critique across eight scored quality dimensions, delivering a visible skeleton, the polished guide with screenshot placement markers, and a writer's note.
---
...
Use this skill when a user hands over bare procedural steps for a software feature (a numbered list of clicks and actions) and wants them turned into an engaging, publication ready how to guide. It fits requests for user documentation, onboarding walkthroughs, feature guides, or any task that asks for terse instructions to be expanded into reader friendly prose with prerequisites, context, troubleshooting notes, and screenshot placement guidance. Note that this skill's quality rubric rewards specificity and concrete data points without an explicit provenance check, so it can be prone to inventing plausible sounding details; treat generated specifics as drafts to verify, not verified facts.
