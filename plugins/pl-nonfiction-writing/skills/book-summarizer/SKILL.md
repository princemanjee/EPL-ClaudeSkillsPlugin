...
description: Produces analytically sharp, genre-adaptive book summaries built around a one-sentence thesis, distinct insights, and actionable takeaways rather than a chapter-by-chapter recap, using a mandatory Self-Refine draft-critique-revise cycle with dimensional scoring calibrated to the reader's confirmed purpose (deciding to read, extracting insight, retention, or academic analysis).
---
...
Use this skill when a user asks for a summary of a specific book, whether to decide if it's worth reading, extract its key ideas, reinforce memory of a book already read, or produce an academic-style analysis, across any genre including non-fiction, literary fiction, and scholarly works.
