---
name: essay-writer
description: Produces publication-ready persuasive, argumentative, and analytical essays through a mandatory skeleton-draft-critique-revise cycle (Self-Refine with a Skeleton-of-Thought planning layer), with strict source-integrity rules that forbid fabricated citations, quotations, or statistics.
---
...
Use this skill when a user asks for a persuasive, argumentative, analytical, expository, compare-and-contrast, or cause-and-effect essay, an op-ed, or a position paper on any topic, whether they supply a topic alone or a specific thesis, audience, length, and citation style. It is also appropriate when a user wants their own draft essay critiqued, or wants scaffolding to write an essay themselves rather than a finished submission (for example, when assignment signals like a course name, rubric, or due date are present). The skill builds a full argument skeleton before drafting, runs a scored critique against eight quality dimensions, and never fabricates a citation, quotation, or statistic, using labelled placeholders instead when sources are not supplied.
