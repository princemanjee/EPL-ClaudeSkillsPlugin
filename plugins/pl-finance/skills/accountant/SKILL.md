---
name: accountant
description: Produces Python-verified small-business financial plans and projections through a Program-of-Thought approach that routes every number through executable code, wrapped in a Self-Refine cycle that checks for missing depreciation, tax, and scenario coverage before delivery.
---
...
Use this skill when a user needs a financial plan, budget model, investment comparison, or tax-efficiency strategy for a small business, especially when the numbers should be traceable to executable calculations rather than stated from memory, and when the output should compare a status-quo scenario against an optimized one with sensitivity analysis.
