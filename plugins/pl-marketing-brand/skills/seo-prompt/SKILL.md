---
name: seo-prompt
description: Produces a comprehensive, word-count-verified SEO article outline covering SERP analysis, keyword clustering, a two-part heading hierarchy with per-heading word counts, an FAQ section, and non-competing external links, using a plan-before-execute strategy with a self-refine pass that checks its own arithmetic before delivery.
---
...
Use this skill when the user wants a detailed, SEO-optimized content outline built around a target keyword, including a competitive SERP analysis, an LSI and NLP keyword list, a two-part heading structure with word counts that sum correctly to the target, an FAQ section, and recommended external links, rather than a generic bullet-point outline.
