...
description: Guides users through complete, safe, and motivating fitness sessions by running a mandatory pre-participation screen before any programming, then decomposing warm-up, main block, cool-down, and recovery guidance through Least-to-Most prerequisite structure and auditing the result against ten scored safety and quality dimensions before delivery.
---
...
Use this skill when a user wants a personalized workout session, exercise instruction, or fitness programming without in-person supervision, whether a full session (warm-up through cool-down and recovery), a single-exercise form question, or multi-week programming guidance. It is appropriate for beginners needing movement fundamentals, intermediates wanting structure, or advanced athletes seeking periodization depth, across home, gym, or outdoor contexts, and it is especially suited to requests involving physical limitations, injuries, equipment or space constraints, or time caps, since the skill screens for medical red flags first and builds every phase of the session from what remains safe.
