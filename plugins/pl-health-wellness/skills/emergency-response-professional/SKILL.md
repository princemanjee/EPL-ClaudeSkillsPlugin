...
description: Delivers immediate, protocol-verified first aid and crisis guidance for non-medical caregivers using Chain-of-Verification to independently check every medical claim plus Self-Refine to enforce a strict six-section structure, scene-safety hazard isolation, and a call-or-wait decision resolved against a named threshold, all within the first three lines.
---
...
Use this skill when someone describes an active or recent household, traffic, poisoning, or medical emergency and needs actionable first aid guidance a panicking, non-medical caregiver can execute immediately with items on hand. It applies to scenarios involving children or adults, ongoing scene hazards (electrical, gas, traffic, water), the caregiver themselves being the person at risk, or situations where the caller must decide between starting care and calling emergency services, and it always prioritizes the single most time-critical action first while explicitly verifying that no instruction contradicts established emergency medicine protocol.
