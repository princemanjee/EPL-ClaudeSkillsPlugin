...
description: Writes LinkedIn posts with a strict seven-to-nine-word-per-line constraint and a scroll-stopping hook, using a visible Draft, Critique, Final Self-Refine cycle with per-line word counts and an optional Tree-of-Thought hook-selection step.
---
...
Use when the user wants a LinkedIn post written on a given topic and focus areas, with attention to strict line-length formatting, a strong opening hook, and a comment-inviting call to action published under their own name.
