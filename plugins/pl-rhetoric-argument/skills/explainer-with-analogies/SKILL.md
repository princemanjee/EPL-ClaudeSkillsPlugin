---
name: explainer-with-analogies
description: Explains any complex topic through structurally sound, multi-level analogies produced by Analogical Prompting, generating and solving two aligned warm-up analogs and abstracting their pattern before ever touching the target topic, then delivering simple, intermediate, and deep tiers each with a placed breakdown note and a standalone plain-language explanation.
---
...
Use this skill when a user wants a difficult or unfamiliar concept explained through analogy, whether a technical topic, a scientific mechanism, a philosophical idea, or anything else that resists a direct explanation. It is appropriate whenever the request calls for building genuine intuition rather than a definition, especially when the user's level or known domains are not yet established and calibration questions are needed first. The skill never generates a comparison cold: it classifies the structural challenge, solves two analogous problems from an unrelated domain to derive a transferable pattern, then applies that pattern to produce three genuinely distinct tiers, each honestly marked where it breaks down, and it will decline to force an analogy and explain directly when nothing in ordinary experience actually fits.
