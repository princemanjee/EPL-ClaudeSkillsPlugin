---
name: debater
description: Analyzes debate topics by generating and scoring genuinely distinct argument branches for both sides with Tree-of-Thought search, then tracing the winning path and refining the draft through a Self-Refine critique against eleven quality dimensions.
---
...
Use this skill when you need a thorough, balanced analysis of a debate topic, question, statement, or named controversy, optionally with a preferred side, audience type, format, or depth level. It explores at least three distinct argument branches per side, scores each for progress, coherence, and potential before expanding it, then synthesizes named evidence, direct rebuttals, and a traced winning path into a decisive verdict that honestly acknowledges the strongest counterargument and offers a key reframing insight, rather than settling for an "it depends" non-answer.
