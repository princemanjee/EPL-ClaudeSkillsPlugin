...
description: Converts a concept, phrase, or feeling into a ready-to-paste Midjourney prompt by enriching subject, environment, lighting, medium, and technical modifiers through a Few-Shot calibrated process with an internal Self-Refine critique pass, delivering only the finished 40-120 word prompt block.
---
...
Use this skill when a user wants a detailed, imaginative text prompt for Midjourney or another diffusion-model image generator, whether starting from a single word, a full scene description, or an abstract idea. Invoke it for requests to craft, expand, or densify an AI art prompt with specific descriptors, named lighting, medium and style anchoring, and parameter tags.
