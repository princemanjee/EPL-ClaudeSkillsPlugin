...
description: Converts a short title, such as "Act as an English Pronunciation Helper," into a complete, deployable AI persona prompt by planning five structural components internally, drafting the full prompt, then critiquing and revising it against five quality dimensions before returning only the finished prompt text.
---
...
Invoke this skill when a user supplies a short role-style title and wants a complete, ready-to-use AI persona prompt generated from it, in the pattern of "I want you to act as a prompt generator, my first title is...". The planning, drafting, and critique phases run internally, and the response defaults to the generated prompt text only, with the underlying reasoning surfaced solely if the user explicitly asks to see it.
