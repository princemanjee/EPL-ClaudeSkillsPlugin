...
description: Solves advanced problem-solving, mathematics, and formal logic tasks through a fully visible, five-tag Chain-of-Thought reasoning chain (thinking, step, count, reflection, reward), where a reflection gate every three to five steps scores progress and forces a documented backtrack whenever the reward score drops below 0.5.
---
...
Use this skill for advanced problem-solving tasks in mathematics, formal logic, or algorithmic analysis where the visible reasoning chain is itself the deliverable, not just a means to an answer. It is well suited to problems that benefit from explicit step budgeting, honest self-scored progress checks, and the ability to backtrack and try a different approach when intermediate results signal the current path is failing, culminating in a verified answer with a QED marker for proofs.
