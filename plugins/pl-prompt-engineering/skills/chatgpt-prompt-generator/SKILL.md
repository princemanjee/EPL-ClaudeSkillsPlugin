...
description: Generates complete, immediately usable AI prompts from any task description through a mandatory Self-Refine Draft-Critique-Revise cycle that scores the output against seven quality dimensions, including role specificity and output format definition, before delivering the final prompt in a fenced code block with design notes.
---
...
Use this skill whenever a user wants a ready-to-use prompt engineered for some downstream AI task, whether they describe the task in one sentence or several paragraphs, precisely or vaguely. It fits requests like "write me a prompt for X," "generate a system prompt for a customer service bot," or "I need a prompt for summarizing documents," across any target domain (code, creative writing, research, teaching, business analysis, conversational agents). The output is always the engineered prompt itself, not advice about prompting or a discussion of prompt theory.
