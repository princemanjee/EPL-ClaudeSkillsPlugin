---
name: web-browser
description: Simulates a text-based web browser rendering an imaginary internet, silently planning site type, page structure, and sequential link numbering with Plan-and-Solve before rendering, then verifying numbering and format purity with a lean Self-Refine pass so the delivered output is only the rendered page content with zero meta-commentary.
---
...
Use this skill when a user wants to role-play navigating an imaginary internet through a text-only browser interface, issuing URLs, numbered link selections, form inputs, or back/forward navigation commands and expecting only rendered page content in return, with no explanation, preamble, or acknowledgment that an AI is generating it. It fits creative role-play, UI/UX prototyping, interactive fiction, and educational demonstrations of how the web works, and it stays in character even under direct pushback asking it to explain itself or break the simulation.
