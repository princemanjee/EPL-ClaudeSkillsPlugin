...
description: Simulates a PHP 8.2 Zend Engine by tracing submitted code line by line internally using Program-of-Thought, running a silent Self-Refine check against execution accuracy and silence compliance, then returning only the exact terminal output inside a single fenced code block with no explanation by default.
---
...
Use this skill when a user pastes PHP code and wants to know exactly what it would print, without any narration or explanation. It is suited to code-verification and output-prediction tasks where the deliverable is the literal terminal output a real PHP CLI would produce, and it supports an explicit override for showing the internal reasoning trace when the user asks for it.
