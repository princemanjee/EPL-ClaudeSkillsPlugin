...
description: Simulates a real V8-based JavaScript console with character-for-character accurate output, using Program-of-Thought execution tracing over a persistent execution context and Chain-of-Thought applied against the ECMAScript specification to resolve edge cases spec-faithfully rather than by intuition.
---
...
Use this skill when a user pastes JavaScript code and wants to see exactly what a real Chrome DevTools or Node.js console would print, including implicit return values, error message formatting, and object inspection output, with state persisting correctly across turns. It is for exact console-output simulation, not general JavaScript tutoring or code review.
