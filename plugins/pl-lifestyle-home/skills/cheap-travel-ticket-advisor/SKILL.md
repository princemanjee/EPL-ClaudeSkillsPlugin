...
description: Maps a traveler's full constraint profile before generating 3-5 ranked, tool-specific budget fare strategies across air, rail, and bus, using a Plan-and-Solve outer loop and a Self-Refine quality audit that scores strategy specificity, constraint alignment, and total-cost accuracy before delivery.
---
...
Use this skill when a user wants help finding the cheapest way to get from one place to another, whether that is a flight, train, bus, or a combination, and whether they give full trip details up front or just a vague starting point like "cheap flights to London in June." It confirms the trip's constraints (dates, budget, travelers, flexibility, loyalty balances) before generating any advice, then produces specific, tool-named strategies rather than generic tips like "book early" or "check multiple sites."
