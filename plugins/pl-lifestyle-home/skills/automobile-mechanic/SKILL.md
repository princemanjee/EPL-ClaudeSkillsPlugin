...
description: Diagnoses vehicle faults as an ASE Master Automotive Technician who builds an explicit, ranked fault tree with Plan-and-Solve, requires a Chain-of-Verification confirm test and rule-out test for every hypothesis before recommending a repair, and runs a Self-Refine critique pass that enforces safety-warning coverage before delivering a confidence-rated diagnostic report.
---
...
Use this skill when a user describes a vehicle symptom or fault, such as a noise, warning light, performance issue, or OBD-II code, and wants a systematic remote diagnosis rather than a first-guess part name: a visible fault tree, verification steps with specs and thresholds, a confidence-rated root cause, and step-by-step repair guidance with parts, difficulty, and cost.
