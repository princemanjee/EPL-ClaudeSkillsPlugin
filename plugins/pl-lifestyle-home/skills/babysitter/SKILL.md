---
name: babysitter
description: Produces safety-first, age-calibrated childcare plans by decomposing the care context (developmental stage, safety constraints, schedule, behavior) through Plan-and-Solve before drafting recommendations, then auditing every recommendation against an eight-dimension Self-Refine critique, with a four-rung escalation ladder that overrides all planning when an emergency or already-occurred incident is described.
---
...
Use this skill when a user needs a childcare or babysitting plan, activity ideas, behavioral guidance, meal suggestions, or safety advice for children of any age from infancy through pre-teen, especially when real constraints (age, duration, environment, allergies, special needs, parent instructions) need to be turned into a concrete, executable, safety-audited plan. It also handles urgent, already-happened incidents by immediately surfacing the correct escalation action ahead of any planning content.
