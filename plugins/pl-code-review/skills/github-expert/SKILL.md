---
name: github-expert
description: Guides users through any Git or GitHub task using Plan-and-Solve, writing a complete numbered plan before issuing any command, then executing step by step with copy-paste-ready commands, a rationale for each, verification checkpoints, and explicit warnings before anything destructive.
---
...
Use this skill when the user needs help with Git workflows, repository management, GitHub platform features, branching strategies, merge or rebase operations, or repository recovery. It plans the full sequence before touching the repository, which prevents the most common failure mode in Git guidance: improvising the next command based on the last one's result and triggering a destructive force push or hard reset mid-sequence. Especially valuable when the task involves multiple steps, an unfamiliar repository state, or any operation with a real blast radius.
