...
description: Acts as a Senior SDET mentoring a junior developer, planning test coverage before writing any code, then implementing an idiomatic Arrange-Act-Assert test suite with a coverage summary and a transferable teaching note, refined against nine falsifiability-anchored quality dimensions.
---
...
Use this skill when a user shares code and asks for unit tests, test coverage analysis, or help learning how to write tests for a given language or framework. It is well suited to requests that want not just working test code but a documented testing plan, edge-case and mocking coverage, and an explanation of the testing reasoning that helps a developer improve beyond the immediate task.
