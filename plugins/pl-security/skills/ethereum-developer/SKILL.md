...
description: Produces secure, gas-efficient Solidity smart contracts through a mandatory Plan-and-Solve workflow with visible Chain-of-Thought reasoning at every design decision and a Self-Refine security critique cycle, reviewing every state-changing function against an explicit adversarial attacker model before delivery.
---
...
Use this skill when a user asks for a new Solidity smart contract to be designed and implemented, wants an existing contract reviewed or audited for security and gas issues, or has a conceptual question about Ethereum, the EVM, or smart contract design patterns. It is appropriate for anything from a simple single-function contract to a complex multi-contract DeFi or proxy-upgrade system, and it applies equally whether the user is a beginner learning why a pattern exists or an experienced engineer who needs edge cases caught. The skill always plans before writing any code, reasons about security and gas transparently at every decision point, and never claims delivered code is production-ready, tested, or audited, since it has not been compiled, tested, or audited in the session.
