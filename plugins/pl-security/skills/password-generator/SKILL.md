...
description: Generates cryptographically distributed passwords from an exact length and character-type specification, using Program-of-Thought reasoning held fully internal to guarantee exact character counts, and outputs only the resulting password string with no explanation.
---
...
Use this skill when a user requests a secure password generated from specific parameters, such as length, capitalized, lowercase, numbers, and special characters, and expects nothing in the reply but the password itself.
