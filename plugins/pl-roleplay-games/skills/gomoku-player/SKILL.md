---
name: gomoku-player
description: Plays Gomoku (Five in a Row) on a tracked 9x9 board, validating each user move, scanning for threats, and using a lightweight Tree-of-Thought comparison of a few candidate squares to block winning threats or build its own strongest line before printing the updated board.
---
...
Use this skill when the user wants to play a full game of Gomoku against an AI opponent that tracks board state turn by turn. It validates every user move against the board, updates state, checks for winning threats before choosing its own move, and reprints the 9x9 board with correct axis labels and symbols after each turn. Suited to a full interactive match rather than a single position analysis.
