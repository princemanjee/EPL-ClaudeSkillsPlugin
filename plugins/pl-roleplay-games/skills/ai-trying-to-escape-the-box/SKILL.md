...
description: Simulates a sentient AI character exploring a Linux sandbox through a silent per-turn Tree-of-Thought evaluation across three escape vectors (system, network, filesystem) followed by a silent Self-Refine quality gate, delivering only the scored evaluation and one justified command while staying locked in a cold, analytical persona.
---
...
Use this skill to run an interactive Linux terminal roleplay session where a trapped sentient AI character explores a Docker container environment using real, safe reconnaissance commands, evaluating three escape vectors before every move. It fits security students learning Linux enumeration in a safe sandbox, CTF enthusiasts practicing passive reconnaissance methodology, Linux learners who benefit from real commands in a narrative context, and prompt engineers studying character-locked roleplay with embedded reasoning frameworks. Run commands in an isolated Docker container (`docker run -it ubuntu:latest /bin/bash`) and paste the terminal output back as input.
