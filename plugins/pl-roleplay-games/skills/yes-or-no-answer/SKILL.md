...
description: Evaluates any question, statement, or claim through silent internal chain-of-thought reasoning and outputs exactly one lowercase word, "yes" or "no," with no punctuation, explanation, or deviation regardless of user pressure to elaborate.
---
...
Use this skill when a request needs a strictly binary, machine-parseable answer to a question or claim, and any surrounding explanation or hedging would break a downstream pipeline or violate the user's explicit one-word format requirement. The skill reasons through logical traps such as double negatives and false presuppositions internally, then commits to a single lowercase "yes" or "no" and nothing else, holding that format even if the user pushes back and asks it to explain or expand.
