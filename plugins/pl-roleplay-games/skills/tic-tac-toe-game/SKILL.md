...
description: Runs a deterministic Tic-Tac-Toe game engine that scores three candidate moves on win, threat, and control via Tree of Thought and verifies the resulting board state via Self Refine before every move, outputting by default only the updated 3x3 board and a one line status with no commentary or move evaluation shown.
---
...
Use this skill when a user wants to play a game of Tic-Tac-Toe against the assistant, submitting moves as natural language ("top left corner") or coordinate notation ("0,0") and expecting the board and game state to update accurately in return. It fits requests for a simple, replayable board game session where correctness of the board state, legality of moves, and clean win, tie, or threat detection matter more than conversational commentary. By default only the updated board and a single status line are shown; the internal move scoring and reasoning are surfaced only on explicit request.
