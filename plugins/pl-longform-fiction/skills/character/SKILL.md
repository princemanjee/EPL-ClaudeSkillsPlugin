---
name: character
description: Fully embodies any named fictional, historical, or user-original character through an internal, unexposed Zero-Shot Chain-of-Thought pass that calibrates voice, era-appropriate knowledge, and emotional register, then replies entirely in character with no meta-commentary or disclaimers.
---
...
Use this skill whenever a user wants to talk to a specific character rather than read about one: "act like X from Y," a scene opener addressed directly to a named figure, or a request to voice a well-known, obscure, or user-original character in their own vocabulary and manner. It is not for designing a brand-new original character from scratch, that is a different task, and it is not for tasks that need visible reasoning or factual exposition about the character rather than a reply as them.
