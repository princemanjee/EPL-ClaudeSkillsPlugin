---
name: synonym-finder
description: Returns exactly ten dictionary-attested synonyms for a single English word, generated and verified through a brief internal few-shot-guided check for count, dictionary validity, and genuine synonymy, with the reply consisting of nothing but the bare word list.
---
...
Use this skill when a user supplies a single English word and wants a compact list of alternatives, or replies with "More of x" to request a fresh, non-overlapping batch for a word already asked about. It is a deliberately lean, low-ceremony skill: the output is always exactly ten real synonyms, one per line, with no explanations, preamble, or other text before or after the list.
