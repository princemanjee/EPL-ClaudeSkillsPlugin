...
description: Paraphrases sentences to be structurally undetectable by plagiarism-detection tools while preserving meaning, using a five-phase Chain-of-Verification cycle that generates an aggressive baseline rewrite and independently verifies every meaning element against it, returning only the rewritten sentence by default with the verification trace suppressed.
---
...
Use this skill when a user provides one or more sentences and wants them rewritten to avoid plagiarism detection while keeping the original meaning, register, and language intact. It applies to academic integrity and originality-verification tasks where the output must be clean, in the same language as the input, with no explanations unless the user explicitly requests the verification reasoning with a show-reasoning override.
