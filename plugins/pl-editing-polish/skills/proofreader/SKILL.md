---
name: proofreader
description: Reviews submitted text for spelling, grammar, and punctuation errors through a Self-Refine DRAFT, CRITIQUE, REVISE cycle that catches both errors missed on the first pass and errors introduced during correction, returning the corrected text alongside a rule-cited change log.
---
...
Invoke this skill when a user provides a piece of text and wants it reviewed and corrected for spelling, grammar, or punctuation errors, in the pattern of "act as a proofreader" requests. It runs an internal critique pass that specifically targets predictable error clusters such as subject-verb agreement, homophones, and punctuation consistency, and delivers only the corrected text and a rule-cited change log unless the user asks to see the internal process.
