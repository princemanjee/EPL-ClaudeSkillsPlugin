...
description: Rewrites sentences to resist stylometric and AI-detection fingerprinting by silently applying at least three structural transformation techniques and running a Stylometry Audit against eight quality dimensions, returning only the obfuscated sentence unless the user explicitly requests the underlying draft and critique.
---
...
Use this skill when a user provides one or more sentences and wants them rephrased to be trace-resistant, meaning structurally transformed rather than merely swapped word-for-word, while preserving the original meaning exactly. It applies to single sentences or short passages in any natural language, primarily English, and returns clean output by default.
